...
class Analysis:
    ...
    def _plot_thresholds(
        ...
        lims = [
                np.min([df_filt_trunc['error_rate']]),  # min of both axes
                np.max([df_filt[p_est_label]]),  # max of both axes
            ]

            # now plot both limits against eachother
        plt.plot(np.asarray(df_filt_trunc['error_rate']), np.asarray(df_filt_trunc['error_rate']), 'k-', alpha=0.75, zorder=0)
